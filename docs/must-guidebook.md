...
- If you adopt reusable-items catalogs: when you need to find a reusable item quickly, check the `notes-reusable-items-*.md` catalogs (or run `sh "$BAGAKIT_LIVING_DOCS_SKILL_DIR/scripts/living-docs-reusable-items.sh" search '<query>' --root .`).
